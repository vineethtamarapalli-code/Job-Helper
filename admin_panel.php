...
// Fetch Placements (Technically stored in 'jobs' table)
$jobs = [];
try {
...
if (isset($_GET['msg'])) {
    ...
    if ($_GET['msg'] === 'job_added' || $_GET['msg'] === 'job_deleted' || $_GET['msg'] === 'job_updated') $default_tab = 'jobs';
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="bg-slate-50 text-slate-800">
    <div class="flex min-h-screen">
        ...
        <aside class="w-64 bg-slate-900 text-white fixed h-full z-20 overflow-y-auto">
            ...
            <nav class="px-4 space-y-2 mt-4">
                ...
                <!-- Placements Tab (Previously Jobs) -->
                <button onclick="switchTab('jobs')" id="btn-jobs" class="w-full flex items-center p-3 rounded-lg hover:bg-slate-800 text-slate-300 hover:text-white cursor-pointer transition">
                    <i class="fas fa-briefcase w-6"></i><span class="font-medium ml-2">Placements</span>
                </button>
        ...
        <main class="ml-64 flex-1 p-8">
            <?php if(isset($_GET['msg'])): ?>
                <div id="alert-box" class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4 flex justify-between items-center transition-opacity duration-1000">
                    <span>
                        ...
                        <?php if($_GET['msg']=='job_added') echo "Placement posted successfully."; ?>
                        <?php if($_GET['msg']=='job_deleted') echo "Placement deleted successfully."; ?>
                        <?php if($_GET['msg']=='job_updated') echo "Placement updated successfully."; ?>
                    </span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                ...
                <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                    <p class="text-sm text-slate-500">Active Placements</p><p class="text-2xl font-bold text-blue-600"><?php echo count($jobs); ?></p>
                </div>
                <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
            ...
            <div id="jobs-section" class="hidden">
                <div class="max-w-4xl mx-auto space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
                        <h3 class="font-bold text-slate-800 text-lg mb-4"><i class="fas fa-plus-circle text-green-500"></i> Post New Placement</h3>
                        <form action="admin_handler.php" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="col-span-2 md:col-span-1">
                                <label class="block text-sm font-medium text-slate-700 mb-1">Placement Title</label>
                            ...
                            <div class="col-span-2 md:col-span-1">
                                <label class="block text-sm font-medium text-slate-700 mb-1">Apply Date</label>
                                <input type="date" name="apply_date" class="w-full p-2 border rounded" required>
                            </div>
                            ...
                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-slate-700 mb-1">Placement URL (Optional)</label>
                                <input type="url" name="job_url" class="w-full p-2 border rounded" placeholder="https://...">
                            </div>

                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-slate-700 mb-1">Placement Description</label>
                                <textarea name="description" rows="3" class="w-full p-2 border rounded" placeholder="Enter placement details..."></textarea>
                            </div>
                            ...
                            <div class="col-span-2 mt-2">
                                <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 font-medium w-full md:w-auto">
                                    <i class="fas fa-paper-plane mr-2"></i> Post Placement
                                </button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                        <div class="p-4 border-b border-slate-100 bg-slate-50 font-bold text-slate-700">Active Placements</div>
                        <div class="p-4 space-y-3">
                            <?php if(count($jobs) == 0): ?>
                                <p class="text-center text-slate-500">No placements posted yet.</p>
                            <?php else: ?>
                                    ...
                                    <div class="flex items-center justify-between p-3 border rounded-lg hover:bg-slate-50 transition">
                                        <div>
                                            ...
                                            <div class="text-sm text-slate-500">
                                                ...
                                                <span class="text-red-500"><i class="fas fa-hourglass-end mr-1"></i> Ends: <?php echo !empty($job['end_date']) ? date('M d', strtotime($job['end_date'])) : 'N/A'; ?></span>
                                            </div>
                                        </div>
                                        
                                        <div class="flex items-center">
                                            <!-- EDIT BUTTON -->
                                            <button onclick="openEditJobModal(this)"
                                                ...
                                                data-url="<?php echo htmlspecialchars($job['job_url']); ?>"
                                                data-desc="<?php echo htmlspecialchars($job['description']); ?>"
                                                data-apply="<?php echo htmlspecialchars($job['apply_date']); ?>"
                                                data-end="<?php echo htmlspecialchars($job['end_date']); ?>"
                                                class="text-blue-500 hover:text-blue-700 p-2 mr-2" title="Edit Placement">
                                                <i class="fas fa-edit"></i>
                                            </button>

                                            <!-- DELETE BUTTON -->
                                            <form action="admin_handler.php" method="POST" onsubmit="return confirm('Delete this placement?');" class="inline">
                                                <input type="hidden" name="action" value="delete_job">
                                                <input type="hidden" name="id" value="<?php echo $job['id']; ?>">
                                                <button type="submit" class="text-red-500 hover:text-red-700 p-2" title="Delete Placement">
            ...
            <div id="users-section">
                <div class="bg-gradient-to-r from-slate-800 to-slate-900 rounded-xl p-6 text-white mb-8 flex items-center justify-between shadow-lg">
                    ...
                    <div class="flex gap-4">
                        <button onclick="switchTab('jobs')" class="px-4 py-2 bg-emerald-600 rounded hover:bg-emerald-700 transition flex items-center"><i class="fas fa-briefcase mr-2"></i>Post Placement</button>
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="text-sm divide-y divide-slate-100">
                            ...
                            <tr>
                                ...
                                <td class="p-4"><span class="bg-blue-100 text-blue-700 py-1 px-3 rounded-full text-xs font-bold"><?php echo $u['f_count']; ?></span></td>
                                <td class="p-4"><span class="bg-purple-100 text-purple-700 py-1 px-3 rounded-full text-xs font-bold"><?php echo $u['s_count']; ?></span></td>
    ...
    <div id="editJobModal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4 backdrop-blur-sm overflow-y-auto">
        <div class="bg-white rounded-xl p-6 w-full max-w-lg my-8">
            <h3 class="font-bold mb-4 text-lg">Edit Placement</h3>
            <form action="admin_handler.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="col-span-2 md:col-span-1">
                        <label class="block text-sm font-medium text-slate-700 mb-1">Placement Title</label>
                        <input type="text" name="title" id="edit_job_title" class="w-full p-2 border rounded" required>
                    </div>
                    
                    <div class="col-span-2 md:col-span-1">
                        <label class="block text-sm font-medium text-slate-700 mb-1">Company / Organization</label>
                        <input type="text" name="company_name" id="edit_job_company" class="w-full p-2 border rounded" required>
                    </div>

                    <div class="col-span-2 md:col-span-1">
                        <label class="block text-sm font-medium text-slate-700 mb-1">Apply Date</label>
                        <input type="date" name="apply_date" id="edit_job_apply" class="w-full p-2 border rounded" required>
                    </div>
                    
                    <div class="col-span-2 md:col-span-1">
                        <label class="block text-sm font-medium text-slate-700 mb-1">End Date</label>
                        <input type="date" name="end_date" id="edit_job_end" class="w-full p-2 border rounded" required>
                    </div>

                    <div class="col-span-2">
                        <label class="block text-sm font-medium text-slate-700 mb-1">Placement URL</label>
                        <input type="url" name="job_url" id="edit_job_url" class="w-full p-2 border rounded">
                    </div>

                    <div class="col-span-2">
                        <label class="block text-sm font-medium text-slate-700 mb-1">Description</label>
                        <textarea name="description" id="edit_job_desc" rows="3" class="w-full p-2 border rounded"></textarea>
                    </div>

                    <div class="col-span-2">
                        <label class="block text-sm font-medium text-slate-700 mb-1">New Document (Optional)</label>
                        <input type="file" name="job_file" class="w-full border rounded p-1 text-sm">
                        <p class="text-xs text-slate-500 mt-1">Leave empty to keep existing document.</p>
                    </div>
                </div>

                <div class="mt-6 flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('editJobModal').classList.add('hidden')" class="px-4 py-2 border rounded text-slate-600 hover:bg-slate-50">Cancel</button>
    ...
    <script>
        ...
        function openEditJobModal(btn) {
            ...
            document.getElementById('edit_job_title').value = btn.dataset.title;
            document.getElementById('edit_job_company').value = btn.dataset.company;
            document.getElementById('edit_job_url').value = btn.dataset.url;
            document.getElementById('edit_job_desc').value = btn.dataset.desc;
            document.getElementById('edit_job_apply').value = btn.dataset.apply;
            document.getElementById('edit_job_end').value = btn.dataset.end;
